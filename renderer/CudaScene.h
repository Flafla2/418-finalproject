...
namespace CudaOpcodes {
    ...
    std::vector<char> refToBytecode(std::vector<RefPrimitive *> const& prims, bool emitBytecode = false);
}

class CudaScene {
public:
    explicit CudaScene(std::vector<RefPrimitive *> const& prims, bool emitBytecode = false);
    ~CudaScene();
